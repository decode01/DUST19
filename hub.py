...
class EmergencyServiceInNeed:
    ...
    def allocateVehicle(self):
        ...
        while True:
            vname = None
            etamin = float("inf")
            if (datetime.now() - self.time).seconds > 10 and allocate == False:
        ...
        try:
            while True:
                ...
                if details[0] == 'H':
                    ...
                    threading.Thread(target=self.activeListenPatient, kwargs={'obj': h_list[len(h_list) - 1]},
                                     daemon=True).start()
                else:
                    a_list.append(tmp_obj)
                    threading.Thread(target=self.activeListenAmb, kwargs={'obj': a_list[len(a_list) - 1]},
                                     daemon=True).start()
        finally:
            self.soc.close()
